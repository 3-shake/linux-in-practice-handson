...
SSM Session Manager で VM に接続(SSH 鍵・公開ポートなし)
  ...
  └ start-chapter chNN で章を切り替え(その章のディレクトリに移動。復習・先取りも自由)
```
...
| `tools/e2e`・`tools/preflight`・`tools/connected` | 運営用（章の実 VM E2E・ローカル機械チェック・接続中の参加者一覧） |
...
第1回（2026-09-16、ch01・ch02）の反省を反映した手順。

1. **接続確認は本編と切り離す。** 開始前に Slack で接続テストを促し、誰が
   つながっているかは `tools/connected` で見る。ウォームアップ問題の時間が
   接続トラブルに食われ、詰まっているのか見ているだけなのか分からなかった。
2. **章を切り替えるときは、コマンドとリンクをチャットに貼る。** 画面共有で
   見せただけでは誰も追っておらず、前の章の問題を解き続けた。
   `start-chapter ch02` をそのまま貼り、問題順のページのリンクも送る
   （切り替え後はその章のディレクトリへ自動で移動する）。
3. **解説は参加者主導にする。** 各自が好きに解く → 解けた人に解き方を
   話してもらう → 解けなくても全員が一言（感想・詰まった点）。運営が一方的に
   進めるとレスポンスが薄く、楽しめているか分からない。
4. 時間内に解けなくても、最後に想定解のライブ実演で「解けた状態」で終える。
...
- 章の切り替えは `start-chapter chNN`（初回はその場で setup が走る。
  切り替え時に前の章は自動で一時停止し、進捗は残る）。対話シェルでは
  切り替えと同時にその章のディレクトリへ移動する（`/etc/profile.d/handson.sh`
  の関数。前の章のディレクトリに居残って別の章の問題を解いてしまう事故を防ぐ）。
  一時停止中・未開始の章のディレクトリに居ると再開方法を1行案内する
  （cd で勝手に切り替えはしない）。
...
壊れた参加者の分だけをピンポイントで再作成できる（他の VM には触れない）：

```console
$ terraform apply -replace='aws_instance.handson["shimomura"]'
```

フラグは同じ値が再計算されるため提出済みフラグはそのまま有効。失われるのは
VM ローカルの状態（修復の成果・作業ファイル）だけで、採点サーバー側の提出記録は残る。
...
- 今日の章は接続した時点で始まっている。前の章の復習や先取りは
  `start-chapter chNN`（進捗は章ごとに残る。実行するとその章のディレクトリに移動する）。
