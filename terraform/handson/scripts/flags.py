#!/usr/bin/env python3
"""Terraform の external data source から呼ばれ、人別フラグを全章分事前計算する。
...
def main():
    ...
    chapters = json.loads(query["chapters"])
    ...
    for chapter in chapters:
        for qid in qids:
            digest = hmac.new(
                query["secret"].encode(),
                f'{query["participant"]}:{chapter}-{qid}'.encode(),
                hashlib.sha256,
            ).hexdigest()
            out[f"{chapter}-{qid}"] = f"flag{{{digest[:HEX_LEN]}}}"
